...
def mediana(I):
    """
    Esta función aplica un filtrado a los pixeles de color negro de la imagen y les aplica la formula de la mediana
    a los nueve pixeles más cercanos. Luego sustituye el el valor del pixel negro por el valor resultante.
    ...
    for x in range(rows):
        for y in range(columns):
            if Y[x, y] == 0:
                ...
                meanValue = np.uint8(np.median(window))
                Y[x, y] = meanValue

    Y = Image.fromarray(Y, 'L')
    Y.save("parte1_p2.jpg")
    return Y
